...
def convert_to_parquet():
    ...
    if not os.path.exists(f"{DATASETS_DIR}/bike_data.parquet"):
        ...
        db.execute(f"""
            COPY (
                SELECT 
                    COALESCE(tripduration, CAST(date_diff('second', CAST(started_at AS TIMESTAMP), CAST(ended_at AS TIMESTAMP)) AS INT)) AS tripduration,
                    COALESCE(starttime, started_at) AS starttime,
                    COALESCE(stoptime, ended_at) AS stoptime,
                    COALESCE(CAST("start station id" AS VARCHAR), CAST(start_station_id AS VARCHAR)) AS "start station id",
                    COALESCE("start station name", start_station_name) AS "start station name",
                    ...
                    COALESCE(CAST("end station id" AS VARCHAR), CAST(end_station_id AS VARCHAR)) AS "end station id",
                    COALESCE("end station name", end_station_name) AS "end station name",
                    TRY_CAST(COALESCE("end station latitude", end_lat) AS DOUBLE) AS "end station latitude",
                    TRY_CAST(COALESCE("end station longitude", end_lng) AS DOUBLE) AS "end station longitude",
                    COALESCE(CAST(bikeid AS VARCHAR), CAST(ride_id AS VARCHAR)) AS bikeid,
                    COALESCE(usertype, member_casual) AS usertype,
                    "birth year",
                    gender
