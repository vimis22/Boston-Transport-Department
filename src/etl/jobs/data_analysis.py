...
from data_analysis.utils.write_parquet_stream import write_parquest_stream
from data_analysis.utils.write_analytics_stream import write_analytics_stream
from data_analysis.utils.write_to_kafka_with_avro import write_to_kafka_with_avro
...
def main():
    ...
    try:
        ...
        weather_df = parse_weather_stream(
            weather_raw, weather_schema
        )  # Now includes enrichment!
        ...
        if config.ENABLE_WEATHER_TRANSPORT_CORRELATION:
            ...
            correlation_query = write_analytics_stream(
                correlation_df, "weather_transport_correlation", output_mode="append"
            )
        ...
        if config.ENABLE_WEATHER_SAFETY_ANALYSIS:
            ...
            safety_df = calculate_weather_safety_risk(
                accident_df, weather_df, window_duration=config.WINDOW_DURATION_LONG
            )
            safety_query = write_analytics_stream(
                safety_df, "weather_safety_analysis", output_mode="append"
            )
        ...
        if config.ENABLE_SURGE_WEATHER_CORRELATION:
            ...
            surge_query = write_analytics_stream(
                surge_df, "surge_weather_correlation", output_mode="append"
            )
        ...
        if config.ENABLE_TRANSPORT_USAGE_SUMMARY:
            ...
            summary_df = generate_transport_usage_summary(
                bike_df, taxi_df, window_duration=config.WINDOW_DURATION_LONG
            )
            summary_query = write_analytics_stream(
                summary_df, "transport_usage_summary", output_mode="append"
            )
        ...
        if config.ENABLE_PEARSON_CORRELATIONS:
            ...
            pearson_query = write_analytics_stream(
                pearson_df, "pearson_correlations", output_mode="append"
            )
        ...
        if config.ENABLE_BINNED_AGGREGATIONS:
            ...
            binned_query = write_analytics_stream(
                binned_df, "weather_binned_metrics", output_mode="append"
            )
        ...
        if config.ENABLE_PRECIPITATION_ANALYSIS:
            ...
            precip_query = write_analytics_stream(
                precip_df, "precipitation_impact", output_mode="append"
            )
        ...
        if config.ENABLE_TEMPORAL_CORRELATIONS:
            ...
            temporal_query = write_analytics_stream(
                temporal_df, "temporal_correlations", output_mode="append"
            )
        ...
        if config.ENABLE_MULTI_VARIABLE_SUMMARY:
            ...
            multi_var_query = write_analytics_stream(
                multi_var_df, "multi_variable_summary", output_mode="append"
            )
        ...
        if config.ENABLE_ACCIDENT_WEATHER_CORRELATION:
            ...
            accident_weather_df = calculate_accident_weather_correlation(
                combined_df, accident_df
            )
