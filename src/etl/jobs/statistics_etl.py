...
def main():
    ...
    try:
        ...
        logger.info("\nKeeping job alive to maintain streaming queries...")

        # Keep the job running to maintain streaming queries
        import signal
        import time

        def signal_handler(sig, frame):
            logger.info("\n=== Received termination signal, stopping queries ===")
            query1.stop()
            query2.stop()
            query3.stop()
            query4.stop()
        ...
        # Wait indefinitely (queries run in Spark Connect server)
        try:
            while True:
                time.sleep(60)
                # Log status every minute
                logger.info(f"Statistics ETL alive - Queries active: {query1.isActive}, {query2.isActive}, {query3.isActive}, {query4.isActive}")
        except KeyboardInterrupt:
            logger.info("\n=== KeyboardInterrupt received, stopping queries ===")
            query1.stop()
            query2.stop()
            query3.stop()
            query4.stop()
        ...
        logger.error(f"Error in Statistics ETL: {e}", exc_info=True)
        raise
    finally:
        logger.info("\n=== Statistics ETL shutdown ===")
