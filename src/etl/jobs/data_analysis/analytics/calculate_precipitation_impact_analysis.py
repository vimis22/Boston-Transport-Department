...
def calculate_precipitation_impact_analysis(combined_df: DataFrame) -> DataFrame:
    ...
    result = combined_df.withColumn(
        ...
        "precipitation_binary", when(col("avg_weather_score") < 40, 1).otherwise(0)
    )
    ...
    result = result.withColumn(
        ...
        when(
            col("precipitation_binary") == 1, -1 * (col("avg_weather_score") - 100) / 2
        ).otherwise(0),  # Higher negative impact when worse weather
    ).withColumn(
        "precip_taxi_boost_score",
        when(
            col("precipitation_binary") == 1, (100 - col("avg_weather_score")) / 2
        ).otherwise(0),  # Taxi usage increases
    )
    ...
    result = result.withColumn(
        ...
        when(
            (col("precipitation_binary") == 1) & (col("taxi_mode_share_pct") > 60), True
        ).otherwise(False),
    )
