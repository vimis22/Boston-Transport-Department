...
def calculate_binned_weather_aggregations(combined_df: DataFrame) -> DataFrame:
    ...
    result = combined_df.withColumn(
        "temp_bin_numeric",
        when(col("avg_temperature_c").isNull(), -999).otherwise(
            (col("avg_temperature_c") / 5).cast("int") * 5
        ),  # Round to nearest 5
    ).withColumn(
        "temp_bin_label",
    ...
    result = result.withColumn(
        "wind_bin_numeric",
        when(col("avg_wind_speed_ms").isNull(), -999).otherwise(
            (col("avg_wind_speed_ms") / 2).cast("int") * 2
        ),  # Bins of 2 m/s
    ).withColumn(
        "wind_bin_label",
    ...
    result = result.withColumn(
        "bikes_per_degree",
        when(
            (col("avg_temperature_c").isNotNull()) & (col("avg_temperature_c") != 0),
            col("total_bike_rentals") / col("avg_temperature_c"),
        ).otherwise(None),
    ).withColumn(
        "bikes_per_weather_score_point",
        when(
            (col("avg_weather_score").isNotNull()) & (col("avg_weather_score") > 0),
