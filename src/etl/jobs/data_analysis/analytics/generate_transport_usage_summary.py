...
def generate_transport_usage_summary(
    bike_df: DataFrame, taxi_df: DataFrame, window_duration: str = "1 hour"
) -> DataFrame:
    """
    ...
    bike_hourly = (
        bike_df.groupBy(window(col("start_time_ts"), window_duration))
        .agg(
        ...
        .withColumn("hour_of_day", hour_func(col("window_start")))
    ...
    taxi_hourly = (
        taxi_df.groupBy(window(col("datetime_ts"), window_duration))
        .agg(
    ...
    # Join bike and taxi
    combined = bike_hourly.join(
        taxi_hourly,
        ["window_start", "window_end", "hour_of_day", "day_of_week"],
        "full_outer",
    )
    ...
    combined = combined.withColumn(
        "total_transport_count", col("bike_count") + col("taxi_count")
    )
    ...
    combined = combined.withColumn(
        "is_weekend",
        when(
            col("day_of_week").isin([1, 7]), True
        ).otherwise(False),  # 1 = Sunday, 7 = Saturday in dayofweek()
    )
