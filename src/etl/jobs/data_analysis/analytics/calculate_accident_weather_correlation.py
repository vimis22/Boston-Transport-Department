...
from pyspark.sql.functions import col, window, count, avg, when, lit, hour as hour_func


def calculate_accident_weather_correlation(
    combined_df: DataFrame, accident_df: DataFrame
) -> DataFrame:
    """
    ...
    accident_agg = (
        accident_df.withWatermark("dispatch_timestamp", "10 minutes")
        .groupBy(window(col("dispatch_timestamp"), "15 minutes"), "mode_type")
        .agg(
            count("*").alias("accident_count"),
            count(when(col("location_type") == "Intersection", 1)).alias(
    ...
    result = (
        ...
        .withColumn(
            "is_freezing", when(col("avg_temperature_c") < 0, True).otherwise(False)
        )
        .withColumn(
            "is_high_wind",
    ...
    result = result.withColumn(
        ...
        + when(col("avg_weather_score") < 30, 15).otherwise(
            0
        ),  # +15 if very poor visibility
    )
    ...
    result = result.withColumn(
        ...
        when(
            (col("mode_type") == "bike") & col("has_precipitation"),
            "high_vulnerability",
        )
    ...
    result = (
        result.withColumn(
            ...
            + when(col("is_freezing"), 1.2).otherwise(
                0.0
            )  # 120% increase when freezing
            + when(col("is_high_wind"), 0.5).otherwise(
                0.0
            ),  # 50% increase in high wind
        )
        .withColumn(
        ...
        .withColumn(
            ...
            col("baseline_accident_rate")
            * col("expected_accident_multiplier")
    ...
    result = result.withColumn(
        ...
        when(
            (col("weather_accident_risk_score") >= 85)
            | (col("accident_rate_per_1000_trips") > 10),
            True,
        ).otherwise(False),
    )
