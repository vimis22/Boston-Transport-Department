...
def calculate_multi_variable_correlation_summary(combined_df: DataFrame) -> DataFrame:
    ...
    result = (
        combined_df.withColumn(
            ...
            when(
                col("total_bike_rentals") > 0, col("total_bike_rentals") / 100.0
            ).otherwise(0),  # Normalize to scale 0-N
        )
        .withColumn(
            "taxi_usage_normalized",
            when(
                col("total_taxi_rides") > 0, col("total_taxi_rides") / 50.0
            ).otherwise(0),  # Different scale for taxis
        )
        .withColumn(
            "temperature_normalized",
            when(
                col("avg_temperature_c").isNotNull(),
                (col("avg_temperature_c") + 10) / 40.0,
            ).otherwise(0.5),  # Normalize -10°C to 30°C to 0 to 1
        )
        .withColumn(
            "wind_normalized",
            when(
                col("avg_wind_speed_ms").isNotNull(), col("avg_wind_speed_ms") / 20.0
            ).otherwise(0),  # Normalize 0-20 m/s to 0 to 1
        )
        .withColumn(
    ...
    result = result.withColumn(
        ...
        when(
            (col("avg_temperature_c").isNotNull())
            & (col("avg_wind_speed_ms").isNotNull()),
    ...
    result = result.withColumn(
        ...
        when(
            (col("avg_temperature_c").between(15, 25))
            & (col("avg_wind_speed_ms") < 8)
            & (col("avg_weather_score") > 70),
            True,
        ).otherwise(False),
    ).withColumn(
        "strong_negative_weather_for_bikes",
        when(
            (col("avg_temperature_c") < 0)
            | (col("avg_wind_speed_ms") > 12)
            | (col("avg_weather_score") < 30),
            True,
        ).otherwise(False),
    )
    ...
    result = (
        ...
        .withColumn("actual_bike_demand_index", col("bike_usage_normalized") * 100)
        .withColumn(
