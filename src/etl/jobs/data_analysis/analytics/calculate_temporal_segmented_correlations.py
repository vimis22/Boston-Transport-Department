...
def calculate_temporal_segmented_correlations(combined_df: DataFrame) -> DataFrame:
    ...
    result = (
        combined_df.withColumn("hour_of_day", hour_func(col("window_start")))
    ...
    result = result.withColumn(
        "temporal_segment",
        when(
            (col("is_weekend") == False) & (col("is_rush_hour") == True),
            "weekday_rush_hour",
        )
        .when(
            (col("is_weekend") == False) & (col("is_rush_hour") == False),
            "weekday_off_peak",
        )
        .when(
            (col("is_weekend") == True) & (col("hour_of_day").between(8, 20)),
            "weekend_day",
        )
